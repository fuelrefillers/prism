...
const createAttendance = asyncHandler(async(req,res,acedemics)=>{
    ...
    stu.forEach(async(s)=>{
        await attenn.create({RollNo:s.RollNo,Regulation:req.body.Regulation,SemesterData:{TotalForSem:acedemics.TotalWorkingDaysForSem*7,ClassesAttendedForSem:acedemics.TotalWorkingDaysForSem*7,HolidaysForSem:acedemics.TotalNumberOfHolidays},MonthlyData:{TotalForMonth:acedemics.WorkingDaysForMonth[0]*7,ClassesAttendedForMonth:acedemics.WorkingDaysForMonth[0]*7,HolidaysForMonth:acedemics.HolidaysForMonth[0]}});
    });
