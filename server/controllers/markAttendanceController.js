...
const setAttendance = asyncHandler(async (req, res) => {
  ...
  for (const student of filteredListARRANGED) {    
    ...
    AttendanceHistoryManuplation(req,res,rollNumbers,filteredListARRANGED,req.query.time,filter);
...
const AttendanceHistoryManuplation = asyncHandler(async(req,res,rollNumbers,presentrnos,time,filter)=>{
  const currentDate = new Date();
